...
function FoundationTokenLabel({ token }: { token: FoundationToken }) {
	return <span className="foundation-token-label"><code><TokenPath value={token.id} token={token} /></code>{token.layer === 'semantic' ? <small>{token.meaning}</small> : null}</span>;
}

function FoundationTokenLabelById({ id, tokens }: { id: string; tokens: readonly FoundationToken[] }) {
	const token = tokenById(tokens, id);
	return token ? <FoundationTokenLabel token={token} /> : <code><TokenName id={id} tokens={tokens} /></code>;
}
...
const HEADING_CONTEXT_EXAMPLES = [
	{ tag: 'h1', title: 'Designing with type', body: 'Typography gives content structure. A clear scale helps readers find their way.' },
	{ tag: 'h2', title: 'Build a clear hierarchy', body: 'Start with the main idea, then move from broad topics to specific details.' },
	{ tag: 'h3', title: 'Organise related ideas', body: 'Group content under a heading that describes what comes next.' },
	{ tag: 'h4', title: 'Make details easy to find', body: 'Use the next level when a section needs a smaller subsection.' },
	{ tag: 'h5', title: 'Keep the structure consistent', body: 'The visual scale follows the same rhythm at every level.' },
	{ tag: 'h6', title: 'Use the level that fits', body: 'Choose headings for the structure of your content.' },
] as const;

function HeadingContext({ data }: { data: FoundationData }) {
	const body = data.typographyRoles.find((role) => role.name === 'body');
	const bodyVariant = body?.variants.find((variant) => variant.name === 'm');
	if (!body || !bodyVariant) throw new Error('Missing typography role body.m.');
	const bodyStyle = styleWithVariables({ fontFamily: cssReference(body.fontFamily), color: cssReference(body.color), fontSize: cssReference(bodyVariant.fontSize), fontWeight: cssReference(bodyVariant.fontWeight), lineHeight: cssReference(bodyVariant.lineHeight), letterSpacing: cssReference(bodyVariant.letterSpacing) });
	return <section className="foundation-section foundation-heading-context" aria-labelledby="foundation-heading-context-title"><div className="foundation-section-heading"><div><h2 id="foundation-heading-context-title">Reading context</h2><p>Heading roles shown alongside body text at their actual size.</p></div></div><div className="foundation-heading-context-list">{HEADING_CONTEXT_EXAMPLES.map(({ tag, title, body: copy }) => { const Heading = tag; return <div className="foundation-heading-context-row" key={tag}><code className="foundation-heading-context-label">&lt;{tag}&gt;</code><div className="foundation-heading-context-copy"><Heading className="foundation-heading-context-title" role="presentation">{title}</Heading><p style={bodyStyle}>{copy}</p></div></div>; })}</div></section>;
}
...
function FoundationElevationFamily({ family, allTokens }: { family: FoundationFamily; allTokens: readonly FoundationToken[] }) {
	return <section className="foundation-family" data-foundation-family={family.id} aria-labelledby={`foundation-family-${family.id}`}>
		<FoundationFamilyHeading family={family} />
		<div className="foundation-family-rows">{family.tokens.map((token) => <div className="foundation-family-row" data-foundation-token={token.id} key={token.id}><FoundationTokenLabel token={token} /><div className="foundation-family-row-specimen"><div className="foundation-surface" style={{ boxShadow: cssReference(token) }}><span>{token.id.split('.').at(-1)}</span></div></div><small><TokenExpression value={token.defaultCssValue} tokens={allTokens} /></small></div>)}</div>
	</section>;
}
...
function MotionTimingEntry({ token, replay, allTokens, family, appliedValues }: { token: FoundationToken; replay: number; allTokens: readonly FoundationToken[]; family: MotionTimingGroup; appliedValues: AppliedNumericValues }) {
	...
	return <div className="foundation-family-row foundation-timing-row" data-foundation-token={token.id}>
		...
		<div className="foundation-timing-value"><FoundationTokenLabel token={token} />{token.authoredAlias ? <small className="foundation-timing-alias">Alias to <a href={`/foundations/${foundationRouteForToken(token.authoredAlias)}/#${tokenIdForAnchor(token.authoredAlias)}`}><TokenName id={token.authoredAlias} tokens={allTokens} /></a></small> : null}<code>{specimenValue(appliedValues.get(token.id) ?? token.defaultValue)} {token.unit}</code></div>
	</div>;
}
